...
/**
 * Tests the {@link RowKeyInSetFilter} in a live HBase cluster
 *
 * @author jeff@opower.com
 */
public class IntTestRowKeyInSetFilter {
    ...
    @Test
    public void testRowsInSetWithComplextCells() throws Exception {
        byte[] qa = new byte[] { 'a' };
        byte[] qb = new byte[] { 'b' };
        byte[] qc = new byte[] { 'c' };
    ...
    private void runFilteredScanTest(HTable table, byte[] family) throws Exception {
        ...
        Filter filter = new RowKeyInSetFilter(keys.keySet());
        Scan scan = new Scan();
